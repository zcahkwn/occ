...
def plot_collude_2():
    x, y = create_meshgrid()
    z = np.vectorize(lambda n1, n2: Analytical_result(N, [n1, n2]).collude_prob() if n1+n2>=N else np.nan)(x, y)
    create_heatmap(
        x, y, z,
        '$n_1$', '$n_2$',
...
def plot_sigma_2():
    x, y = create_meshgrid()
    z = np.vectorize(lambda n1, n2: Analytical_result(N,[n1,n2]).compute_sigma())(x,y)
    # z = (N*(x+y)-x*y)/(N**2)
    create_heatmap(
        x, y, z,
        '$n_1$', '$n_2$',
...
def plot_occ_relation_2():
    x, y = create_meshgrid()
    z=np.vectorize(lambda n1, n2: Analytical_result(N, [n1, n2]).collude_prob()/Analytical_result(N, [n1, n2]).occ_value())(x, y)
    create_heatmap(
        x, y, z,
...
def plot_sigma_relation_2():
    x, y = create_meshgrid()
    z=np.vectorize(lambda n1, n2: Analytical_result(N, [n1, n2]).collude_prob()/Analytical_result(N, [n1, n2]).compute_sigma())(x, y)
    create_heatmap(
        x, y, z,
